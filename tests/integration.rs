...
impl TestProject {
    fn new() -> Self {
        ...
        fs::write(dir.path().join("src").join("main.rs"), "fn main() { println!(\"hello\"); }\n").unwrap();
...
fn test_list_stale() {
    ...
    p.adocs().arg("init").assert().success();
    p.adocs().arg("sync").assert().success();
    p.adocs().arg("list").arg("--state").arg("stale").assert().success()
...
fn test_list_valid_after_update() {
    ...
    p.adocs().arg("sync").assert().success();
    p.adocs().arg("update").arg("src/main.rs").assert().success();
    p.adocs().arg("list").arg("--state").arg("valid").assert().success()
        .stdout(predicate::str::contains("src/main.rs"));
}
...
fn test_stale_alias() {
    ...
    p.adocs().arg("init").assert().success();
    p.adocs().arg("sync").assert().success();
    p.adocs().arg("stale").assert().success()
...
fn test_seal_refuses_stale() {
    ...
    p.adocs().arg("init").assert().success();
    p.adocs().arg("sync").assert().success();
    p.adocs().arg("seal").arg("src/main.rs")
        .assert().failure()
...
fn test_list_sealed() {
    ...
    p.adocs().arg("list").arg("--state").arg("sealed").assert().success()
        .stdout(predicate::str::contains("src/main.rs"));
}
...
fn test_status_json() {
    ...
    p.adocs().arg("init").assert().success();
    p.adocs().arg("sync").assert().success();
    p.adocs().arg("status").arg("--json").assert().success()
...
fn test_changed_reports_new_files() {
    ...
    p.adocs().arg("changed").assert().success()
...
fn test_rename_preserves_state() {
    ...
    p.adocs().arg("sync").assert().success();
    p.adocs().arg("update").arg("src/main.rs").assert().success();
    fs::rename(p.path().join("src").join("main.rs"), p.path().join("src").join("app.rs")).unwrap();
    p.adocs().arg("status").assert().success()
...
fn test_install_agent_outputs_config() {
    let p = TestProject::new();
    p.adocs().arg("install-agent").arg("opencode").assert().success()
...
fn test_docsunder_shows_valid_files() {
    ...
    p.adocs().arg("sync").assert().success();
    p.adocs().arg("update").arg("src/main.rs").assert().success();
    p.adocs().arg("docsunder").arg("src").assert().success()
        .stdout(predicate::str::contains("src/main.rs"))
        .stdout(predicate::str::contains("valid"));
}
...
fn test_docsunder_filesonly_excludes_folders() {
    ...
    p.adocs().arg("sync").assert().success();
    p.adocs().arg("update").arg("src/main.rs").assert().success();
    p.adocs().arg("docsunder").arg("src").arg("--filesonly").assert().success()
        .stdout(predicate::str::contains("src/main.rs"));
}
...
fn test_docsunder_json_output() {
    ...
    p.adocs().arg("sync").assert().success();
    p.adocs().arg("update").arg("src/main.rs").assert().success();
    p.adocs().arg("docsunder").arg("src").arg("--json").assert().success()
...
mod mcp_tests {
    ...
    fn call(server: &AdocsMcpServer, name: &str, args: serde_json::Map<String, serde_json::Value>) -> serde_json::Value {
    ...
    fn test_mcp_read_context_stale() {
        ...
        let result = call(&server, "adocs_read_context", serde_json::from_str(r#"{"path":"src/main.rs"}"#).unwrap());
    ...
    fn test_mcp_read_file_description() {
        ...
        let result = call(&server, "adocs_read_file_description", serde_json::from_str(r#"{"path":"src/main.rs"}"#).unwrap());
    ...
    fn test_mcp_read_folder_purpose() {
        ...
        let result = call(&server, "adocs_read_folder_purpose", serde_json::from_str(r#"{"path":"src"}"#).unwrap());
        let raw = result.get("raw").and_then(|v| v.as_str()).unwrap_or("");
    ...
    fn test_mcp_update_doc_promotes_to_valid() {
        ...
        let result = call(&server, "adocs_update_doc", serde_json::from_str(r#"{"path":"src/main.rs"}"#).unwrap());
        assert_eq!(result["state"], "valid");

        let ctx = call(&server, "adocs_read_context", serde_json::from_str(r#"{"path":"src/main.rs"}"#).unwrap());
    ...
    fn test_mcp_explain_staleness() {
        ...
        let result = call(&server, "adocs_explain_staleness", serde_json::from_str(r#"{"path":"src/main.rs"}"#).unwrap());
    ...
    fn test_mcp_request_seal_asks_human() {
        ...
        let result = call(&server, "adocs_request_seal", serde_json::from_str(r#"{"path":"src/main.rs"}"#).unwrap());
        let raw = result.get("raw").and_then(|v| v.as_str()).unwrap_or("");
    ...
    fn test_mcp_read_folder_docs() {
        ...
        let result = call(&server, "adocs_read_folder_docs", serde_json::from_str(r#"{"path":"src"}"#).unwrap());
        assert_eq!(result["folder"], "src");
        let docs = result["docs"].as_array().unwrap();
        assert!(docs.iter().any(|d| d["path"] == "src/main.rs" && d["kind"] == "file"));
    }
    ...
    fn test_mcp_read_folder_docs_filesonly() {
        ...
        let result = call(&server, "adocs_read_folder_docs", serde_json::from_str(r#"{"path":"src","files_only":true}"#).unwrap());
        let docs = result["docs"].as_array().unwrap();
    ...
    fn test_mcp_read_folder_docs_foldersonly() {
        ...
        let result = call(&server, "adocs_read_folder_docs", serde_json::from_str(r#"{"path":"src","folders_only":true}"#).unwrap());
        let docs = result["docs"].as_array().unwrap();
